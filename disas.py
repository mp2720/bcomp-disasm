...
class AddrOrImmInstr(Instr):
    ...
    class Mode(Enum):
        ...
        Перед загрузкой [IP+OPND] уменьшается на 1
        """
...
def disas_instr(cur_addr: int, word: int) -> tuple[Instr, int | None]:
    ...
    def io() -> tuple[IOInstr, None] | None:
        mnemonics = {0x0: "DI", 0x1: "EI", 0x2: "IN", 0x3: "OUT", 0x8: "INT"}
    ...
    def addr_or_imm() -> tuple[AddrOrImmInstr, int | None] | None:
        ...
        if bit11:
            ...
            return AddrOrImmInstr(
                mnemonic, word, AddrOrImmInstr.Mode.ABSOLUTE_DIRECT, addr
            ), addr
    ...
    def branch() -> tuple[AddrOrImmInstr, int] | None:
        ...
        return AddrOrImmInstr(
            mnemonics[ext_opcode], word, AddrOrImmInstr.Mode.IP_RELATIVE_DIRECT, addr
        ), addr
...
def dump_prog(
    ...
    def place_org(addr: int):
        if addr is not None:
            lines.append(f"            {trans('ORG'):8}0x{addr:03x}")
    ...
    def place_instr(instr: Instr, addr: int):
        ...
        match instr:
            case ImplInstr():
                line += tr_mnemonic
            case IllegalInstr():
                line += f"{trans('WORD'):8}0x{instr.word:04x}"
            case _:
                line += f"{tr_mnemonic:8}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f",
        "--input-format",
        help="По умолчанию lab - в формате варианта лабораторной РАБоты, "
        "hex - набор шестнадцатиричных цифр, которые могут быть разделены пробельными символами, "
        "bin - в бинарном виде",
        nargs="?",
        default="lab",
    )
    ...
    if args.filepath is not None:
        file = open(args.filepath, "r" + ("b" if args.input_format == "bin" else ""))
    else:
    ...
    match args.input_format:
        ...
        case "hex":
            ...
            if len(text) & 1:
                raise ValueError("Количество шестнадцатеричных цифр должно быть чётным")

            data = bytearray.fromhex(text)
            instrs_addrs = range(start_addr, start_addr + len(data) // 2)
        case "lab":
            LINE_RE = re.compile(r"^([0-9a-fA-F-]+)\s*:\s*([0-9a-zA-Z-]+)$")
            ...
            for i, line in enumerate(cast(str, file.read()).split("\n")):
                ...
                addr_str, instr_str = m.group(1), m.group(2)

                if addr_str == "---" and instr_str == "----":
                    # skip separator
                    continue

                if instr_str[0].lower() in "ghijklmnopqrstuvwxyz":
                    continue
